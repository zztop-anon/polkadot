...
function createKey(registry, itemFn, keys, hashers, args) {
  ...
  return compactAddLength(u8aConcat(createPrefixedKey(itemFn), ...keys.map((type, index) => {
...
export function createFunction(registry, itemFn, options) {
  ...
  const storageFn = expandWithMeta(itemFn, arg => type.isDoubleMap ? createKey(registry, itemFn, [type.asDoubleMap.key1, type.asDoubleMap.key2], [getHasher(type.asDoubleMap.hasher), getHasher(type.asDoubleMap.key2Hasher)], arg) : type.isMap ? createKey(registry, itemFn, [type.asMap.key], [getHasher(type.asMap.hasher)], [arg]) : options.skipHashing ? compactAddLength(u8aToU8a(options.key)) : createKey(registry, itemFn, [], [], []));
